  ...
  return (
    <div className="landing">
      ...
      {page === "Ana sayfa" ? <>
        ...
        <section className="landing-cards">{[[Landmark, "E-Şube İşlemleri", "Hisse al-sat, limit emir, portföy pozisyonu ve geçmiş akışı tek panelde."], [Newspaper, "Piyasa ve Haber", "BIST, SPK, TCMB ve piyasa haberleri arama ve detay ekranlarıyla izlenir."], [ShieldCheck, "Operasyon Güvenliği", "KYC, KVKK, sözleşme, para hareketi ve admin step-up kontrolleri birlikte çalışır."]].map(([Icon, title, text]) => <article key={title}><Icon /><h3>{title}</h3><p>{text}</p></article>)}</section>
        <section className="landing-band split"><h2>Ottoman ile işlemler net, kontrol merkezi güçlü.</h2><p>Kullanıcı tarafında sade bir yatırım akışı, admin tarafında müşteri, emir, para, risk, T+2 ve bakiye kontrolünü yöneten tam yetkili operasyon masası bulunur.</p></section>
