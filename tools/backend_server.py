...
""",
    ...
    def serve_file(self, path: Path) -> None:
        ...
        self.send_header("Content-Length", str(len(body)))
        # index.html önbellekte kalırsa tarayıcı eski JS/CSS paketini süresiz
        # çalıştırır; kullanıcı yeni sürümü hiç görmez. HTML hiç saklanmaz,
        # hash'li paketler ise kalıcı olarak saklanır.
        if path.suffix.lower() in (".html", ".htm"):
            self.send_header("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0")
            self.send_header("Pragma", "no-cache")
            self.send_header("Expires", "0")
        elif "assets" in path.parts and re.search(r"-[A-Za-z0-9_-]{8,}\.[A-Za-z0-9]+$", path.name):
